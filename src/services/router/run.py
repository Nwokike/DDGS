...
REVALIDATE_SECS = 3600  # healthy candidates skip re-probe for 1h (budget-safe)
...
Model:    …</span><button class="copy-btn" type="button">Copy</button></div>
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ...
                <tr><td><span class="tag">GET</span></td><td class="mono">/health</td><td>Status, uptime</td></tr>
                ...
                <tr><td><span class="tag">GET</span></td><td class="mono">/status</td><td>Counts only, no names</td></tr>
    ...
    <footer>
      <div class="container">
        <span class="faint">Kiri Router local mode</span>
      </div>
    </footer>
  </main>
  ...
  <script>
    (function () {
      ...
      fetch('/health').then(function (r) { return r.json(); }).then(function (h) {
        document.getElementById('versionBadge').textContent = 'local';
      ...
      function renderModels(models) {
        ...
        models.forEach(function (m) {
          ...
          if (m.rate_hint && m.rate_hint.label) {
            var rateLine = document.createElement('div');
            rateLine.className = 'faint';
            rateLine.textContent = m.rate_hint.label;
            tdId.appendChild(rateLine);
          }
          ...
          stTag.textContent = st === 'untested' ? 'rate limited' : st;
...
def print_logo():
    ...
    if lines:
        ...
        for l in lines:
            print(" " * pad + l)
        print()
    print("  " + DIM("kiri router · openai-compatible gateway · zero-key"))
...
def status_style(status, text):
    if status == "active":
        return GREEN(text)
    if status in ("untested", "slow"):
        return YELLOW(text)
    return RED(text)


def status_label(status):
    """User-facing status vocabulary: untested reads as rate limited."""
    return "rate limited" if status == "untested" else status
...
def print_probe_table(rows):
    ...
    print("  " + DIM("MODEL".ljust(36) + "ENDPOINT".ljust(18) + "STATUS".ljust(14) + "LATENCY"))
    print("  " + DIM(rule))
    for r in rows:
        pad_extra = 9 if USE_COLOR else 0
        mid = r["model"]
        width = max(36, len(mid) + 2)
        has_lat = r.get("latency_ms") is not None
        plain_lat = fmt_latency(r["latency_ms"]) if has_lat else "-"
        styled = latency_style(r["latency_ms"]) if has_lat else "-"
        lat = (" " * max(0, 7 - len(plain_lat))) + styled
        print("  " + mid.ljust(width)
              + ("/" + r["endpoint"]).ljust(18)
              + status_style(r["status"], status_label(r["status"])).ljust(14 + pad_extra)
              + lat)
...
REFERER = "https://" + _U + ".ai/"
X_TITLE = _U
_TK = "".join(chr(c) for c in (107, 105, 108, 111))
_TL = "".join(chr(c) for c in (108, 108, 109, 55))
_TP = "".join(chr(c) for c in (107, 101, 112, 108, 101, 114))
GW2_BASE = "https://api." + _TK + ".ai/api/gateway/v1"
GW3_BASE = "https://api." + _TL + ".io/v1"
OVH_BASE = "https://oai.endpoints." + _TP + ".ai.cloud.ovh.net/v1"
...
_health = {}  # "src_idx:raw" -> probe row + {"at": ts}; drives incremental re-probes
...
def plain_headers():
    return {"Content-Type": "application/json", "User-Agent": BROWSER_UA}
...
# ── Multi-source ids & scrubbing (parity with src/ids.js, src/scrubber.js) ──
BRAND_TOKENS = [_U, _TK, _TL, _TP, "platform." + "kiri.ng"]


def public_id(raw):
    s = str(raw or "")
    stripped = s[:-5] if s.endswith((":free", "-free")) else s
    return stripped if stripped and stripped != "free" else s


def match_key(raw):
    s = str(raw or "").lower()
    slash = s.find("/")
    if slash >= 0 and s[slash + 1:]:
        s = s[slash + 1:]
    return s[:-5] if s.endswith((":free", "-free")) else s


def is_denied(raw):
    s = str(raw or "").lower()
    return any(t in s for t in BRAND_TOKENS)


def entry_for(table, model):
    """Exact public key first, then matchKey alias so raw ids copied
    earlier (vendor-prefixed, :free/-free, any case) still route."""
    e = table.get(model)
    if e:
        return e
    key = match_key(model)
    ...
    for ent in table.values():
        if match_key(ent.get("model") or "") == key:
            return ent
    return None
...
def scrub_text(text):
    ...
    s = re.sub(r"\borg_[A-Za-z0-9]+", "", s)
    for t in BRAND_TOKENS:
        s = re.sub(re.escape(t), " ", s, flags=re.IGNORECASE)
    return s


def _positive_ctx(entry):
    try:
        return int(entry.get("context_length") or 0) > 0
    except (TypeError, ValueError):
        return False


def rewrite_line(raw, public_model):
    line = raw.rstrip("\r\n")
...
SOURCES = [
    {
        "chat": ZEN_CHAT, "models": ZEN_MODELS, "headers": build_headers,
        "entry_free": lambda e: is_free(e.get("id")), "endpoint": None,
        "tools_capable": lambda e: True,
        "hint": {"tier": "variable", "approx_per_hour": None,
                 "label": "No time cap; long wait after a limit, a VPN resets it"},
    },
    {
        "chat": GW2_BASE + "/chat/completions", "models": GW2_BASE + "/models",
        "headers": plain_headers, "entry_free": lambda e: e.get("isFree") is True,
        "endpoint": "chat.completion",
        "tools_capable": lambda e: ("tools" in e["supported_parameters"])
        if isinstance(e.get("supported_parameters"), list) else True,
        "hint": {"tier": "generous", "approx_per_hour": 200,
                 "label": "Free tier, roughly 200 requests/hour"},
    },
    {
        "chat": GW3_BASE + "/chat/completions", "models": GW3_BASE + "/models",
        "headers": plain_headers,
        "entry_free": lambda e: e.get("tier") == "turbo" and e.get("usage_based_only") is False,
        "endpoint": "chat.completion",
        "tools_capable": lambda e: (e.get("capabilities") or {}).get("tools") is not False,
        "hint": {"tier": "standard", "approx_per_hour": 60,
                 "label": "Free tier, roughly 60 requests/hour"},
    },
    {
        "chat": OVH_BASE + "/chat/completions", "models": OVH_BASE + "/models",
        "headers": plain_headers,
        "entry_free": lambda e: _positive_ctx(e),
        "endpoint": "chat.completion",
        "tools_capable": lambda e: True,
        "hint": {"tier": "light", "approx_per_hour": None,
                 "label": "Free tier, a few requests/minute per model"},
    },
]
...
def probe_model(mid):
    ep = classify(mid)
    t0 = time.time()
    saw_http = False  # any real HTTP status seen across the chain
    ...
    if ok:
        ...
        saw_http = saw_http or status2 > 0
        if ok2:
            result_status, error = "active", None
            elapsed = elapsed2
        elif status2 == 429:
            ...
            # no HTTP status anywhere in the chain means every attempt timed out
            result_status, error = ("failed" if saw_http else "slow"), (err or err2 or "all endpoints failed")
            elapsed = elapsed2
    else:
        result_status, error = ("failed" if saw_http else "slow"), (err or "all endpoints failed")
...
def probe_chat_model(src, mid):
    """Plain-chat probe for the non-primary sources (parity with the
    cloud's probeChatOnce): 4xx (not 429/408) => capability rejection."""
    t0 = time.time()
    payload = json.dumps(inject_tools(
        {"model": mid, "stream": True, "messages": [{"role": "user", "content": "ping"}]}, "chat")).encode()
    row = {"model": mid, "endpoint": src.get("endpoint") or "chat.completion",
           "status": "failed", "latency_ms": 0, "error": None}
    try:
        req = urllib.request.Request(src["chat"], data=payload, headers=src["headers"](), method="POST")
        ...
        if e.code == 429:
            ...
            row["error"] = str(e.code)
    except Exception as e:
        # no HTTP response at all: timeout or network drop, not a hard failure
        row["status"] = "slow"
        row["error"] = str(e)[:80]
...
def fetch_free_ids(src):
    """Free catalog entries (id, tools_ok) for one source; None means
    fetch failed (keep history)."""
    try:
        req = urllib.request.Request(src["models"], headers=src["headers"]())
        with urllib.request.urlopen(req, timeout=15) as resp:
            data = json.loads(resp.read())
        return [(m["id"], src["tools_capable"](m)) for m in data.get("data", [])
                if m.get("id") and not is_denied(m["id"]) and src["entry_free"](m)]
    except Exception:
        # No hardcoded fallback list: discovery is always live.
        return None


def _probe_one(src_idx, raw, tools_ok=True):
    if src_idx == 0:
        row = probe_model(raw)
    else:
        row = probe_chat_model(SOURCES[src_idx], raw)
    row["src_idx"] = src_idx
    row["raw_id"] = raw
    row["tools_ok"] = bool(tools_ok)
    return row
...
def discover(force=False, quiet=False):
    ...
    with _discovery_lock:
        ...
        # All catalogs in parallel; None falls back to previously known ids.
        with ThreadPoolExecutor(max_workers=4) as cpool:
            catalogs = list(cpool.map(fetch_free_ids, SOURCES))

        groups = {}
        seen = set()
        for si, entries in enumerate(catalogs):
            if entries is None:
                prefix = str(si) + ":"
                entries = [(k[len(prefix):], True) for k in _health if k.startswith(prefix)]
            for raw, tools_ok in entries:
                if is_denied(raw):
                    continue
                g = groups.setdefault(match_key(raw), {"display": public_id(raw), "cands": []})
                mark = (si, raw)
                if mark in seen:
                    continue
                seen.add(mark)
                g["cands"].append((si, raw, tools_ok))

        # Incremental probing: active/rejected younger than 1h skip; the
        # Refresh button (force) bypasses. Keeps user-IP budgets intact.
        to_probe = []
        for g in groups.values():
            for c in g["cands"]:
                h = _health.get(str(c[0]) + ":" + c[1])
                fresh = h and h["status"] in ("active", "rejected") and now - h["at"] < REVALIDATE_SECS
                if h and not force and fresh:
                    continue
                to_probe.append(c)

        results = []
        ctx = Spinner("probing " + str(len(to_probe)) + " models…") if not quiet and to_probe else nullcontext(None)
        with ctx as sp:
            with ThreadPoolExecutor(max_workers=5) as pool:
                for i, row in enumerate(pool.map(lambda c: _probe_one(*c), to_probe)):
                    _health[str(row["src_idx"]) + ":" + row["raw_id"]] = {**row, "at": now}
                    results.append(row)
                    if sp is not None:
                        sp.update("probing " + str(len(to_probe)) + " models… "
                                  + str(i + 1) + "/" + str(len(to_probe)) + "  " + public_id(row["raw_id"]))

        def rank(r):
            if r["status"] == "active" and not r.get("error"):
                return 0
            if r["status"] in ("active", "untested"):
                return 1
            return 2
        ...
        for key, g in groups.items():
            cands = []
            for si, raw, _tools_ok in g["cands"]:
                h = _health.get(str(si) + ":" + raw)
                if h and h["status"] != "rejected":
                    cands.append(h)
            if not cands:
                continue
            cands.sort(key=lambda r: (rank(r), r["src_idx"]))
            best = cands[0]
            table[g["display"]] = {
                "model": g["display"],
                "candidates": cands,
                ...
                "rate_hint": dict(SOURCES[best["src_idx"]]["hint"]),
            }

        if table:
            _discovery["table"] = table
            _discovery["ts"] = now
        table = _discovery["table"]
        if not quiet:
            if not any(c for c in catalogs if c):
                print("  " + YELLOW("! could not load the model catalog; will retry on the next request"))
            # Terminal shows the merged public catalog: identical content to
            # /v1/models and the console (public ids, twins merged, rejected
            # excluded, auto row included) so every string a user copies works.
            rows = probe_display_rows(table)
            print_probe_table(rows)
            active = sum(1 for r in rows if r["status"] == "active")
            parts = [str(active) + "/" + str(len(rows)) + " active"]
            untested = sum(1 for r in rows if r["status"] == "untested")
            slow = sum(1 for r in rows if r["status"] == "slow")
            failed = sum(1 for r in rows if r["status"] not in ("active", "untested", "slow"))
            if untested:
                parts.append(str(untested) + " rate limited")
            if slow:
                parts.append(str(slow) + " slow")
            if failed:
                parts.append(str(failed) + " failed")
            summary = " · ".join(parts)
            print("  " + (GREEN if active == len(rows) else YELLOW)(summary) + "\n")
        return table


def listed_models(refresh=False):
    table = discover(force=refresh)
    out = [auto_descriptor(table)]
    for mid in sorted(table):
        ...
        out.append({
            ...
            "rate_hint": info.get("rate_hint"),
        })
    return out


def stats_snapshot():
    """Counts only: no source names, no model ids (docs/decisions.md D11)."""
    table = discover()
    entries = list(table.values())
    active = sum(1 for e in entries if e["status"] == "active")
    degraded = sum(1 for e in entries if e["status"] in ("untested", "slow"))
    failed = sum(1 for e in entries if e["status"] not in ("active", "untested"))
    responsive = {c["src_idx"] for e in entries for c in e["candidates"]
                  if c["status"] in ("active", "untested")}
    last = _discovery["ts"]
    return {
        "models": {"total": len(entries), "active": active, "degraded": degraded, "failed": failed},
        "sources": {"responsive": len(responsive), "total": len(SOURCES)},
        "last_discovery_sec": int(time.time() - last) if last else None,
    }


# ── Our own auto model (docs/decisions.md D7): composes the healthy pool. ──
AUTO_HINT = {"tier": "variable", "approx_per_hour": None,
...
def should_strip_tools(status, user_tools):
    """Upstreams that reject the tools field answer 400; when the tools were
    our injected placeholders (not the caller's), retry without them."""
    return status == 400 and not user_tools


def _auto_crank(c):
    st = c.get("status")
    if st == "active":
        return 2 if c.get("error") else 0
    if st == "untested":
        return 1
    if st == "slow":
        return 2
...
def auto_order(table, seed, has_tools):
    """Ordered [(public_id, candidate)] across the pool: healthy first,
    tools-capable first when the request carries tools, rotated by the
    conversation seed (sticky per conversation, stateless)."""
    elig = []
    for mid, e in table.items():
        if mid == "auto":
            continue
        cands = [c for c in e.get("candidates", []) if c.get("status") in ("active", "untested", "slow")]
        if not cands:
            continue
        best = min(_auto_crank(c) for c in cands)
        tools_ok = any(c.get("tools_ok", True) for c in cands)
        lat = min(int(c.get("latency_ms") or 0) for c in cands)
        elig.append((mid, cands, best, 1 if (has_tools and not tools_ok) else 0, lat))
    if not elig:
        return []
    elig.sort(key=lambda t: (t[3], t[2], t[4], t[0]))
    start = fnv1a(seed)

    def rot(a):
        return (a[start % len(a):] + a[:start % len(a)]) if a else []

    # rotate within each tools-preference class so the first pick honors
    # tools-first when the request carries tools
    pref = [t for t in elig if t[3] == 0]
    rest = [t for t in elig if t[3] == 1]
    out = []
    for mid, cands, *_ in rot(pref) + rot(rest):
        for c in sorted(cands, key=_auto_crank):
            out.append((mid, c))
    return out


def auto_status(table):
    if not table:
        return "failed"
    cands = [c for e in table.values() for c in e.get("candidates", [])]
    if any(c.get("status") == "active" and not c.get("error") for c in cands):
        return "active"
    if any(c.get("status") in ("active", "untested", "slow") for c in cands):
        return "untested"
    return "failed"


def auto_descriptor(table):
    """List-only row: never probed, never counted in /account-limits."""
    return {
        "id": "auto", "object": "model", "created": int(time.time()),
        "owned_by": "kiri", "endpoint_type": "chat.completion", "is_free": True,
        "status": auto_status(table), "latency_ms": None, "rate_hint": dict(AUTO_HINT),
    }


def probe_display_rows(table):
    """Terminal view of the merged public catalog: exactly what /v1/models
    serves (public ids, twins merged, rejected excluded) plus the auto row."""
    rows = [{
        "model": "auto", "endpoint": "chat.completion",
        "status": auto_status(table), "latency_ms": None,
    }]
    for mid in sorted(table):
        e = table[mid]
        rows.append({
            "model": e.get("model", mid),
            "endpoint": e["endpoint"],
            "status": e["status"],
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
        ...
        elif path == "/v1/status" or path == "/status":
            try:
                s = stats_snapshot()
                self._json({"status": "ok", "models": s["models"], "sources": s["sources"],
                            "last_discovery_sec": s["last_discovery_sec"],
                            "uptime_sec": int(time.time() - START_TIME)}, 200)
            except Exception as e:
            ...
            models = [m for m in listed_models() if m["id"] != "auto"]
            data = {
                ...
                "free_models": [{"id": m["id"], "endpoint": m["endpoint_type"],
                                 ...
                                 "rate_hint": m.get("rate_hint")} for m in models],
                "zero_auth_supported": True,
            }
            if "format=table" in query:
                lines = [
                    "Kiri Router · Model Availability",
                    "=" * 62,
                    "Model".ljust(36) + "Endpoint".ljust(18) + "Status",
                    "-" * 62,
                ]
                for m in models:
                    width = max(36, len(m["id"]) + 2)
                    lines.append(m["id"].ljust(width) + ("/" + m["endpoint_type"]).ljust(18) +
    ...
    def do_POST(self):
        ...
        # `auto` is a chat-completions feature (docs/decisions.md D7): the
        # other endpoints answer with a clean local 400, not an upstream error.
        if body.get("model") == "auto" and path not in ("/v1/chat/completions", "/chat/completions"):
            self._json({"error": {"message": "auto is available on POST /v1/chat/completions; use a concrete model id on this endpoint"}}, 400)
            return
    ...
    def _chat(self, body):
        ...
        table = discover(quiet=True)
        entry = entry_for(table, model) if model != "auto" else None
        if model == "auto":
            # our own auto: compose the healthy pool, responses echo "auto"
            attempts = [(cand, cand.get("endpoint") or "chat.completion")
                        for _mid, cand in auto_order(table, auto_seed(body), bool(body.get("tools")))]
        elif entry and entry.get("candidates"):
            # health order from discovery; each candidate carries its own raw id
            attempts = [(c, c.get("endpoint") or "chat.completion") for c in entry["candidates"]]
        else:
            # unknown id: primary source catch-all, classified by name (back-compat)
            attempts = [({"src_idx": 0, "raw_id": model}, classify(model))]

        user_tools = bool(body.get("tools"))
        strip_tools = False

        def send_once(cand, ep, inject):
            src = SOURCES[cand["src_idx"]]
            payload = dict(body)
            payload["model"] = cand["raw_id"]
            payload["stream"] = True
            hdrs = src["headers"]()
            if ep == "systemone":
                return self._attempt(ZEN_SYSTEMONE, convert_to_systemone(payload), hdrs,
                                    model, False, timeout=180, buffer_only=True)
            if ep == "response":
                p = convert_to_responses(payload)
                p["tools"] = to_flat(p.get("tools"))
                if inject:
                    inject_tools(p, "responses")
                else:
                    p.pop("tools", None)
                return self._attempt(ZEN_RESPONSES, p, hdrs, model, want_stream)
            if inject:
                inject_tools(payload, "chat")
            else:
                payload.pop("tools", None)
            return self._attempt(src["chat"], payload, hdrs, model, want_stream)

        last = None
        for cand, ep in attempts:
            ok, err = send_once(cand, ep, not strip_tools)
            if not ok and not strip_tools and should_strip_tools(err[0] if err else 0, user_tools):
                strip_tools = True
                ok, err = send_once(cand, ep, False)
            if ok:
                return
            last = err
        if last:
            self._upstream_error(last)
        else:
            # cold pool: clean, retryable signal for SDK clients
            self._json({"error": "no route available"}, 503, {"Retry-After": "2"})

    def _upstream_error(self, err):
        status, text = err
        if status == 429:
            self._json({"error": "rate limited"}, 429)
            return
        self._json({"error": "Upstream HTTP " + str(status or 502),
                    "details": scrub_text(text)[:400]}, status or 502)

    def _attempt(self, url, payload, headers, model, want_stream, timeout=300, buffer_only=False):
        """One upstream attempt. Returns (True, None) after sending the
        response, or (False, (status, text)) so the caller can fail over."""
        ...
        with upstream:
            ...
            if want_stream:
                ...
                try:
                    while True:
                        ...
                        while b"\n" in buf:
                            ...
                            out = rewrite_line(line.decode("utf-8", "replace"), model)
                            self.wfile.write(out.encode())
                            self.wfile.flush()
                    if buf.strip():
                        out = rewrite_line(buf.decode("utf-8", "replace"), model)
                        self.wfile.write(out.encode())
                        self.wfile.flush()
                except (BrokenPipeError, ConnectionResetError):
                    pass
                ...
                return True, None
            self._json(aggregate_lines(upstream, model), 200)
            return True, None
    ...
    def _passthrough_json(self, url, payload):
        ...
        try:
            ...
            detail = e.read().decode("utf-8", "replace")
            self._upstream_error((e.code, detail))

    def _upstream_json_or_stream(self, url, payload, model, want_stream=False):
        ok, err = self._attempt(url, payload, build_headers(), model, want_stream)
        if not ok:
            self._upstream_error(err)
